...
const NftCreate: NextPage = () => {
  const { ethereum, signedContract } = useWeb3();
  ...
  const handleChange = (
    ...
    setNftMeta((prev) => ({ ...prev, [name]: value }));
  };
  ...
  const handleAttributesChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setNftMeta((prev) => ({ ...prev, attributes: nftMeta.attributes }));
  };
  ...
  const createNft = async () => {
    try {
      ...
      const tx = await signedContract?.mintToken(
        nftURI,
              ...
              <div className="mt-2 text-sm text-yellow-700">
                <p>
                  {network.isLoading ? (
                    ...
                    <button onClick={network.switchToCorrectChain}>
