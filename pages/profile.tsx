...
const Profile: NextPage = () => {
  ...
  useEffect(() => {
    if (nfts.data && nfts.data.length > 0) setActiveNft(nfts.data[0]);
    return () => setActiveNft(undefined);
  }, [nfts.data]);

  const handleChangePrice = (e: ChangeEvent<HTMLInputElement>) => {
    if (!isNaN(+e.target.value)) setNewPrice(e.target.value);
  };

  const handleListNft = async () => {
    if (isList) {
      activeNft && nfts.listNft(activeNft.tokenId, +newPrice);
      setIsList(false);
    } else {
      setIsList(true);
    }
              ...
              {activeNft && (
                <div className="pb-16 space-y-6">
                  ...
                  <div className="flex">
                    ...
                    >
                      {activeNft.isListed ? "Nft Listed" : "List Nft"}
                    </button>
                  </div>
                </div>
              )}
